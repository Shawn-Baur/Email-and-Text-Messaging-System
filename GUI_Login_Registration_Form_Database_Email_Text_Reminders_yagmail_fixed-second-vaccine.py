...
def excel():
    ...
    sheet.cell(row=1, column=13).value = "Perfered arm"
    sheet.cell(row=1, column=14).value = "Egg Allergy"
...
# set focus on the pefered_arm_field box
def focus9(event):
    perfered_arm_field.focus_set()


# set focus on the address_field box
def focus10(event):
    egg_allergy_field.focus_set()
...
def clear():
    ...
    perfered_arm_field.delete(0, END)
    egg_allergy_field.delete(0, END)
...
def first_vaccine():
    ...
    with yagmail.SMTP(from_address, app_password) as yag:
        ...
        print('Sent 1st Covid vaccination email successfully')  #you can have different success message
    ...
    message = client.messages.create(
                        ...
                        to=round(text),  #this is the patient's phone number to receive the text msg
                        )
    ...
    for i, r in df.iterrows():
        ...
        if (
            ...
            if r["Sent"] == 1:
                ...
                with yagmail.SMTP(from_address, app_password) as yag:
                    yag.send(to_address, subject, email_content)
                    print('Sent 2nd Covid vaccination email successfully')  #you can have different success message
                ...
                message = client.messages.create(
                                    ...
                                    from_=Twilio_Number, #this is your own Twilio number
                                    to=r["SendText"], #this is the patient's phone number to receive the text msg
                                    )

                print(message.sid)
                print('Sent 2nd Covid vaccination text successfully')  #you can have different success message
                print("----------------")
...
def date_verify():
    ...
    message = client.messages.create(
                        ...
                        from_=Twilio_Number, #this is your own Twilio number
                        to=Text, #this is the patient's phone number to receive the text msg
                        )
...
def insert():
    ...
    if (
        ...
        or gender_field.get() == ""
        or perfered_arm_field.get() == ""
        or egg_allergy_field.get() == ""
    ):
        Empty_Input()
        ...
        sheet.cell(row=current_row + 1, column=13).value = perfered_arm_field.get()
        sheet.cell(row=current_row + 1, column=14).value = egg_allergy_field.get()
...
def register_form():
    ...
    bg = PhotoImage(file=r"background_img.png", height=400, width=1000)
    ...
    global gender_field
    global perfered_arm_field
    global egg_allergy_field
    ...
    gender_field_in = StringVar()
    perfered_arm_field_in = StringVar()
    egg_allergy_field_in = StringVar()
    ...
    gender = Label(
        root,
        text="gender: Male, Female, Other, Perfer to not say",
        bg="black",
        fg="white",
        font=("Calibri", 13),
    )
    ...
    perfered_arm.grid(row=10, column=0)
    egg_allergy.grid(row=11, column=0)
    ...
    gender_field = Entry(root, textvariable=gender_field_in)
    perfered_arm_field = Entry(root, textvariable=perfered_arm_field_in)
    egg_allergy_field = Entry(root, textvariable=egg_allergy_field_in)
    ...
    perfered_arm_field.bind("<Return>", focus9)
    
    egg_allergy_field.bind("<Return>", focus10)
    ...
    perfered_arm_field.grid(row=10, column=1, ipadx="100")
    egg_allergy_field.grid(row=11, column=1, ipadx="100")
...
def register():
    ...
    # refrenced above
    bg = PhotoImage(file=r"register_img.png", height=800, width=800)
...
def login():
    ...
    bg = PhotoImage(file=r"login_img.png", height=400, width=320)
...
def main_account_screen():
    ...
    bg = PhotoImage(file=r"background_img.png", height=400, width=800)
